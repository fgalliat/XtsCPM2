...
/*
 * @(#)GBZ80.java 1.1 27/04/97 Adam Davidson & Andrew Pollard
 * With hacks by Jack Palevich from Z80.c by Marat Fayzullin
 */

/**
 * <p>The GBZ80 class emulates the Zilog Z80-like microprocessor in the Gameboy.</p>
 *
...
public abstract class GBZ80 {
	...
	// IFF1 is the non-maskable interrupt (0x80)
	// IFF2 is the maskable interrupt (0x01)
	protected boolean    _IFF1 = true, _IFF2 = true;
	protected int        _IM = 2;

	private int Trace_ = 0;
	private int Trap_ = 0xffff;
	private boolean _CPURunning = true;
	
	public void CPURunning(boolean run){
	  _CPURunning = run;
	}
  ...
  abstract int M_RDMEM(int A);
  abstract void M_WRMEM(int A, int V);

	/** Byte access */
	private final int peekb( int addr ) {
	  return M_RDMEM((char) addr);
	}
	public void pokeb( int addr, int newByte ) {
		M_WRMEM((char) addr, (byte) newByte);
	}
	...
	/** Call stack */
	public final void pushpc() { pushw( PC() ); }
	public final void poppc()  { PC( popw() ); }
  ...
  private final int hlinc() {
    int hl = HL();
    HL( inc16( hl ));
    return hl;
  }

  private final int hldec() {
    int hl = HL();
    HL( dec16( hl ));
    return hl;
  }
  ...
  // Return the address to jump to, or ffff if not to go anywhere
  abstract int doInterrupt();
  
	public int interrupt() {
	  int newAddress = doInterrupt();
		if ( !IFF1() && !IFF2() ) {
			return 0;
		}
...
*/
	...
	public final void execute() {
         ...
         while ( _CPURunning ) {
          if(PC() ==Trap_) Trace_= 2;			/*** Turn tracing on if trapped ***/
          if(Trace_ != 0) Debug.Debug(this);		/*** Call single-step debugger  ***/

          if ( interruptTriggered( local_tstates ) ) local_tstates -= tstatesPerInterrupt - interrupt();
          REFRESH( 1 );
          ...
          switch ( nxtpcb() ) {
           ...
           case 8: {					/* Gameboy code. Not EX AF,AF' */
            // J.B.l=M_RDMEM(R.PC.W++);J.B.h=M_RDMEM(R.PC.W++);
            // M_WRMEM(J.W++,R.SP.B.l);M_WRMEM(J.W,R.SP.B.h);
            pokew(nxtpcw(), SP());
            local_tstates += ( 20 );
           break;
           }
    ...
    {
      // LD (0xff00 + #), A
      // M_WRMEM(0xFF00+M_RDMEM(R.PC.W++),R.AF.B.h);
      pokeb(0xff00+nxtpcb(), A());
      local_tstates += ( 12 );
      break;
    }
    case 0xE2:
    {
      // LD (0xff00 + C), A
      // M_WRMEM(0xFF00+R.BC.B.l,R.AF.B.h);
      pokeb(0xff00+C(), A());
      local_tstates += ( 8 );
      break;
    }
    ...
    {
      // LD A,(0xff00+#)
      // R.AF.B.h=M_RDMEM(0xFF00+M_RDMEM(R.PC.W++));
      int value = nxtpcb();
      //if (value == 0 ) {
      //  Trace(1);
      //}
      A(peekb(0xff00+value));
      local_tstates += ( 12 );
      break;
    }
    case 0xF2:
    {
      // LD A,(0xff00+C)
      // R.AF.B.h=M_RDMEM(0xFF00+R.BC.B.l);
      A(peekb(0xff00+C()));
      local_tstates += ( 8 );
      break;
    }
    case 0xF8:
    {
      // R.HL.W=R.SP.W+(offset)M_RDMEM(R.PC.W++);
      HL(SP()+(byte)nxtpcb());
      local_tstates += ( 12 );
      break;
    }
...
// -------------
    ...
    // Gameboy has a SWAP instruction instead of SLS
    case 0x30: // SWAP B
    { B(swap(B())); return ( 8 ); }
    case 0x31: // SWAP C
    { C(swap(C())); return ( 8 ); }
	...
	and_a( int b )
	{
		...
		set5( (ans & F_5) != 0 );
		setH( true );
		// No parity on game boy CPU setPV( parity[ ans ] );
		setZ( ans == 0 );
		setN( false );
		setC( false );
	...
	/** swap high and low nibble - alters no flags */
	private final int
	swap( int ans )
	{
	  // XXX Table lookup?
		return ((ans & 0x0f ) << 4) | ((ans & 0xf0) >> 4);
	}
