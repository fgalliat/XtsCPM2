#ifndef __GFX_GLYPH__
#define __GFX_GLYPH__ 1
...
// for text !!
#define lcd_width SCR_WIDTH
#define lcd_height SCR_HEIGHT

int_least16_t __drawChar(int_least16_t x, int_least16_t y, char c, uint_least16_t color, uint_least16_t bg, uint_least8_t size) {
...
//   size = size & 0x7F;
    ...
    else if ((y + FONT_HEIGHT - 1) >= lcd_height) {
      return lcd_width + 1;
    }

    tft.setAddrWindow(x, y, x + FONT_WIDTH - 1, y + FONT_HEIGHT-1);
    SPI.beginTransaction(SPISET);

    // int xx = 0, yy = 0;
    uint_least8_t height = FONT_HEIGHT;
    uint_least8_t maskStart = (1 << (FONT_WIDTH - 1));
    for (; height != 0; height--) {
      // #if FONT_WIDTH <= 8
      ...
      // #endif
      for (mask = maskStart; mask != 0; mask >>= 1) {
        if (data & mask) {
          //tft.drawPixel(x + xx, y + yy, color);
          writedata16( color );
        }
        else {
          //tft.drawPixel(x + xx, y + yy, bg);
          writedata16( bg );
        }
        // xx++;
      }
