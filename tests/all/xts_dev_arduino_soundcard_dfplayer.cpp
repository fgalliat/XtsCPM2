...
// 	void delay(int time) { usleep(time*1000); }
  ...
  // =============================
	...
	void SoundCard::mute()   { stop(); _mute = true; }
    void SoundCard::unmute() { _mute = false; }
    bool SoundCard::isMute() { return _mute; }


	bool SoundCard::setup() {
		pinMode( MP3BUSY_PIN, INPUT );
		if ( this->serial == NULL) {
			this->serial = &MP3SERIAL;
    ...
    bool SoundCard::isPlaying() {
		_snd_isPlaying = digitalRead( MP3BUSY_PIN ) == LOW;
